...
export function AppleButton({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <a
      href={href}
      className="flex h-12 w-full items-center justify-center gap-2.5 rounded-xl bg-black text-[15px] font-semibold text-white shadow-sm transition-all hover:shadow-md hover:opacity-90"
    >
      ...
      {compact ? (
        <>
          ...
          <span className="hidden sm:inline">המשך עם Apple</span>
        </>
      ) : (
        "המשך עם Apple"
      )}
